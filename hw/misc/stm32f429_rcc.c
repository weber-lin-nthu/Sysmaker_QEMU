/*
 * STM32F429 RCC (Reset and clock control)
 *
...
#include "hw/misc/stm32f429_rcc_internals.h"
#include "hw/clock.h"
#include "hw/irq.h"
...
static void clock_mux_update(RccClockMuxState *mux, bool bypass_source)
{
    ...
    trace_stm32f429_rcc_mux_update(mux->id, mux->src, src_freq,
...
static void clock_mux_set_enable(RccClockMuxState *mux, bool enabled)
{
    ...
    if (enabled) {
        trace_stm32f429_rcc_mux_enable(mux->id);
    } else {
        trace_stm32f429_rcc_mux_disable(mux->id);
    }
                                 ...
                                 uint32_t multiplier, uint32_t divider)
{
    ...
    trace_stm32f429_rcc_mux_set_factor(mux->id,
...
static void clock_mux_set_source(RccClockMuxState *mux, RccClockMuxSource src)
{
    ...
    trace_stm32f429_rcc_mux_set_src(mux->id, mux->src, src);
    mux->src = src;
    clock_mux_update(mux, false);
}
...
static void pll_update(RccPllState *pll, bool bypass_source)
{
    ...
    for (i = 0; i < RCC_NUM_CHANNEL_PLL_OUT; i++) {
        ...
        trace_stm32f429_rcc_pll_update(pll->id, i, vco_freq,
...
static void pll_set_vco_multiplier(RccPllState *pll, uint32_t vco_multiplier)
{
    ...
    trace_stm32f429_rcc_pll_set_vco_multiplier(pll->id,
...
//     if (enabled) {
//         trace_stm32f429_rcc_pll_channel_enable(pll->id, channel);
//     } else {
//         trace_stm32f429_rcc_pll_channel_disable(pll->id, channel);
//     }
                                    ...
                                    uint32_t divider)
{
    ...
    trace_stm32f429_rcc_pll_set_channel_divider(pll->id,
...
// static void rcc_update_msi(Stm32f429RccState *s, uint32_t previous_value)
// {
// uint32_t val;
...
static void rcc_update_cr_register(Stm32f429RccState *s, uint32_t previous_value)
{
    int val;
    const RccClockMuxSource current_pll_src =
        CLOCK_MUX_INIT_INFO[RCC_CLOCK_MUX_PLL_INPUT].src_mapping[s->clock_muxes[RCC_CLOCK_MUX_PLL_INPUT].src];

    /* PLLSAION and update PLLSAIRDY */
    val = FIELD_EX32(s->cr, CR, PLLSAION);
    pll_set_enable(&s->plls[RCC_PLL_PLLSAI], val);
    s->cr = (s->cr & ~R_CR_PLLSAIRDY_MASK) |
            (val << R_CR_PLLSAIRDY_SHIFT);
    // if (s->cier & R_CIER_PLLSAIRDYIE_MASK) {
    //     s->cifr |= R_CIFR_PLLSAIRDYF_MASK;
    // }
    ...
    // if (s->cier & R_CIER_PLLI2SRDYIE_MASK) {
    //     s->cifr |= R_CIFR_PLLI2SRDYF_MASK;
    // }
...
static void rcc_update_cfgr_register(Stm32f429RccState *s)
{
    uint32_t val;
    /* MCO1PRE */
    ...
    clock_mux_set_source(&s->clock_muxes[RCC_CLOCK_MUX_MCO1], val);
    /* MCO2 */
    val = FIELD_EX32(s->cfgr, CFGR, MCO2);
    clock_mux_set_enable(&s->clock_muxes[RCC_CLOCK_MUX_MCO2], true);
    clock_mux_set_source(&s->clock_muxes[RCC_CLOCK_MUX_MCO2], val);
...
// static void rcc_update_ahb1enr(Stm32f429RccState *s)
// {
...
// static void rcc_update_ahb2enr(Stm32f429RccState *s)
// {
...
// static void rcc_update_ahb3enr(Stm32f429RccState *s)
// {
 ...
 */
static void rcc_update_plli2scfgr(Stm32f429RccState *s)
{
    uint32_t reg = s->plli2scfgr, val;

    /* Get R value */
    val = FIELD_EX32(reg, PLLI2SCFGR, PLLI2SR);
    pll_set_channel_divider(&s->plls[RCC_PLL_PLLI2S], RCC_PLL_COMMON_CHANNEL_R,
                            val);

    /* Get Q value */
    val = FIELD_EX32(reg, PLLI2SCFGR, PLLI2SQ);
    pll_set_channel_divider(&s->plls[RCC_PLL_PLLI2S], RCC_PLL_COMMON_CHANNEL_Q,
                            val);

    /* PLLN */
    val = FIELD_EX32(reg, PLLI2SCFGR, PLLI2SN);
    pll_set_vco_multiplier(&s->plls[RCC_PLL_PLLI2S], val);
}

/*
 * Note: no frequency bounds checking is done here.
 */
static void rcc_update_pllsaicfgr(Stm32f429RccState *s)
{
    uint32_t reg = s->pllsaicfgr, val;

    /* Get R value */
    val = FIELD_EX32(reg, PLLSAICFGR, PLLSAIR);
    pll_set_channel_divider(&s->plls[RCC_PLL_PLLSAI], RCC_PLL_COMMON_CHANNEL_R,
                            val);

    /* Get Q value */
    val = FIELD_EX32(reg, PLLSAICFGR, PLLSAIQ);
    pll_set_channel_divider(&s->plls[RCC_PLL_PLLSAI], RCC_PLL_COMMON_CHANNEL_Q,
                            val);

    /* PLLN */
    val = FIELD_EX32(reg, PLLSAICFGR, PLLSAIN);
    pll_set_vco_multiplier(&s->plls[RCC_PLL_PLLSAI], val);
}

static void rcc_update_pllcfgr(Stm32f429RccState *s)
{
    int val;

    /* Get Q value */
    val = FIELD_EX32(s->pllcfgr, PLLCFGR, PLLQ);
    pll_set_channel_divider(&s->plls[RCC_PLL_PLL], RCC_PLL_COMMON_CHANNEL_Q,
                            val);
    ...
    /* Get P value */
    val = FIELD_EX32(s->pllcfgr, PLLCFGR, PLLP);
    pll_set_channel_divider(&s->plls[RCC_PLL_PLL], RCC_PLL_COMMON_CHANNEL_P,
                            2 + 2 * val);
...
// static void rcc_update_bdcr(Stm32f429RccState *s)
// {
//     int val;
...
// static void rcc_update_csr(Stm32f429RccState *s)
// {
//     int val;
                                   ...
                                   unsigned int size)
{
    ...
    trace_stm32f429_rcc_read(addr, retvalue);

    return retvalue;
}

static void stm32f429_rcc_write(void *opaque, hwaddr addr,
                                uint64_t val64, unsigned int size)
{
    ...
    trace_stm32f429_rcc_write(addr, value);
    ...
    case A_CIR:
        qemu_log_mask(LOG_UNIMP,
                      "%s: not implemented for CIR\n", __func__);
        break;
    case A_PLLSAICFGR:
        s->pllsaicfgr = value;
        rcc_update_pllsaicfgr(s);
        break;
    case A_PLLI2SCFGR:
        s->plli2scfgr = value;
        rcc_update_plli2scfgr(s);
        break;
...
static const MemoryRegionOps stm32f429_rcc_ops = {
    .read       = stm32f429_rcc_read,
    .write      = stm32f429_rcc_write,
    .endianness = DEVICE_NATIVE_ENDIAN,
    .valid      = {
             .max_access_size = 4,
             .min_access_size = 4,
             .unaligned       = false},
    .impl = {.max_access_size = 4, .min_access_size = 4, .unaligned = false},
};

static const ClockPortInitArray stm32f429_rcc_clocks = {
    QDEV_CLOCK_IN(Stm32f429RccState, hsi16_rc, NULL, 0),
    QDEV_CLOCK_IN(Stm32f429RccState, msi_rc, NULL, 0),
    QDEV_CLOCK_IN(Stm32f429RccState, hse, NULL, 0),
    QDEV_CLOCK_IN(Stm32f429RccState, lsi_rc, NULL, 0),
    QDEV_CLOCK_IN(Stm32f429RccState, lse_crystal, NULL, 0),
    QDEV_CLOCK_IN(Stm32f429RccState, sai1_extclk, NULL, 0),
    QDEV_CLOCK_IN(Stm32f429RccState, sai2_extclk, NULL, 0),
    QDEV_CLOCK_END};

static void stm32f429_rcc_init(Object *obj)
{
    Stm32f429RccState *s = STM32F429_RCC(obj);
    size_t i;
    ...
    memory_region_init_io(&s->mmio, obj, &stm32f429_rcc_ops, s,
                          TYPE_STM32F429_RCC, 0x400);
    sysbus_init_mmio(SYS_BUS_DEVICE(obj), &s->mmio);

    qdev_init_clocks(DEVICE(s), stm32f429_rcc_clocks);
...
static void connect_mux_sources(Stm32f429RccState *s,
                                ...
                                const RccClockMuxSource *clk_mapping)
{
    ...
    Clock *const CLK_SRC_MAPPING[] = {
        ...
        [RCC_CLOCK_MUX_SRC_PLLCLK] =
            s->plls[RCC_PLL_PLL].channels[RCC_PLL_CHANNEL_PLLCLK],
        [RCC_CLOCK_MUX_SRC_PLLI2SCLK] =
            s->plls[RCC_PLL_PLLI2S].channels[RCC_PLLI2S_CHANNEL_PLLI2SCLK],
        [RCC_CLOCK_MUX_SRC_PLLSAICLK_I2S] =
            s->plls[RCC_PLL_PLLI2S].channels[RCC_PLLI2S_CHANNEL_PLLSAICLK],
        [RCC_CLOCK_MUX_SRC_PLLSAICLK_SAI] =
            s->plls[RCC_PLL_PLLSAI].channels[RCC_PLLSAI_CHANNEL_PLLSAICLK],
        [RCC_CLOCK_MUX_SRC_PLLCDCLK] =
            s->plls[RCC_PLL_PLLSAI].channels[RCC_PLLSAI_CHANNEL_PLLCDCLK],
        ...
        [RCC_CLOCK_MUX_SRC_RTCCLK] =
            s->clock_muxes[RCC_CLOCK_MUX_RTCCLK].out,
    };
...
static const VMStateDescription vmstate_stm32f429_rcc = {
    .name               = TYPE_STM32F429_RCC,
    .version_id         = 1,
    .minimum_version_id = 1,
    .fields             = (VMStateField[]){
        VMSTATE_UINT32(cr, Stm32f429RccState),
        VMSTATE_UINT32(pllcfgr, Stm32f429RccState),
        VMSTATE_UINT32(cir, Stm32f429RccState),
        VMSTATE_UINT32(cfgr, Stm32f429RccState),
        VMSTATE_UINT32(pllsaicfgr, Stm32f429RccState),
        VMSTATE_UINT32(plli2scfgr, Stm32f429RccState),
        // VMSTATE_UINT32(cier, Stm32f429RccState),
        // VMSTATE_UINT32(cifr, Stm32f429RccState),
        // VMSTATE_UINT32(ahb1rstr, Stm32f429RccState),
        // VMSTATE_UINT32(ahb2rstr, Stm32f429RccState),
        // VMSTATE_UINT32(ahb3rstr, Stm32f429RccState),
        // VMSTATE_UINT32(apb1rstr1, Stm32f429RccState),
        // VMSTATE_UINT32(apb1rstr2, Stm32f429RccState),
        // VMSTATE_UINT32(apb2rstr, Stm32f429RccState),
        ...
        // VMSTATE_UINT32(ahb2smenr, Stm32f429RccState),
        // VMSTATE_UINT32(ahb3smenr, Stm32f429RccState),
        // VMSTATE_UINT32(apb1smenr1, Stm32f429RccState),
        // VMSTATE_UINT32(apb1smenr2, Stm32f429RccState),
        // VMSTATE_UINT32(apb2smenr, Stm32f429RccState),
        ...
        VMSTATE_CLOCK(lsi_rc, Stm32f429RccState),
        VMSTATE_CLOCK(lse_crystal, Stm32f429RccState),
        VMSTATE_CLOCK(sai1_extclk, Stm32f429RccState),
        VMSTATE_CLOCK(sai2_extclk, Stm32f429RccState),
        VMSTATE_END_OF_LIST()}};

static void stm32f429_rcc_realize(DeviceState *dev, Error **errp)
{
    Stm32f429RccState *s = STM32F429_RCC(dev);
    size_t i;
...
static Property stm32f429_rcc_properties[] = {
    DEFINE_PROP_UINT64("hse_frequency", Stm32f429RccState,
                       hse_frequency, HSE_DEFAULT_FRQ),
    DEFINE_PROP_UINT64("sai1_extclk_frequency", Stm32f429RccState,
                       sai1_extclk_frequency, 0),
    DEFINE_PROP_UINT64("sai2_extclk_frequency", Stm32f429RccState,
                       sai2_extclk_frequency, 0),
    DEFINE_PROP_END_OF_LIST(),
};

static void stm32f429_rcc_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc     = DEVICE_CLASS(klass);
    ResettableClass *rc = RESETTABLE_CLASS(klass);
    ...
    rc->phases.hold = stm32f429_rcc_reset_hold;
    device_class_set_props(dc, stm32f429_rcc_properties);
    dc->realize = stm32f429_rcc_realize;
    dc->vmsd    = &vmstate_stm32f429_rcc;
}

static const TypeInfo stm32f429_rcc_types[] = {
    {
        .name          = TYPE_STM32F429_RCC,
        .parent        = TYPE_SYS_BUS_DEVICE,
        .instance_size = sizeof(Stm32f429RccState),
        .instance_init = stm32f429_rcc_init,
        .class_init    = stm32f429_rcc_class_init,
    },
    {
        .name          = TYPE_RCC_CLOCK_MUX,
        .parent        = TYPE_DEVICE,
...
DEFINE_TYPES(stm32f429_rcc_types)
